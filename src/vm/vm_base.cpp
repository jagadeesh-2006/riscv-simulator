...
void VmBase::LoadProgram(const AssembledProgram &program)
{
    ...
    // ensure there's a breakpoint at program end to signal program termination
    // but avoid calling AddBreakpoint() which prints errors when duplicate;
    // add silently only if not already present.
    if (!CheckBreakpoint(program_size_))
    {
        breakpoints_.emplace_back(program_size_);
    }
...
// Provide a default Reset implementation for VmBase so derived
// classes can call VmBase::Reset() to initialize common VM state.
void VmBase::Reset()
{
    program_counter_ = 0;
    ...
    // reset memory and registers to a clean state
    registers_.Reset();
    memory_controller_.Reset();

    // keep program_ and program_size_ intact (Reset shouldn't unload program)
}
